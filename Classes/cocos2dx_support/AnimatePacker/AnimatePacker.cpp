...
static AnimatePacker *s_animatePacker = NULL;

AnimatePacker* AnimatePacker::getInstance()
{
	if (s_animatePacker == NULL) {
		s_animatePacker = new AnimatePacker();
	}

	return s_animatePacker;
}
...
void AnimatePacker::loadAnimations(const char *path )
{
	...
	{
	    string plistPath = CCFileUtils::sharedFileUtils()->fullPathFromRelativeFile(plists[i].c_str(), pszPath);//::fullPathFromRelativeFile();
