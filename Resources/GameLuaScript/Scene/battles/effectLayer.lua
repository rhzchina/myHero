...
function M:changeActions(change_data)
	...
	for i = 1 , #change_data do
		...
		-- 显示掉血动画
		local effect_changeHp = require("GameLuaScript/Scene/battle/effect/changeHp")
		...
		if isset(cur_data , "actions") then
			...
			if cur_data["actions"]["dodge"] then
				local heroAction_dodge = require("GameLuaScript/Scene/battle/heroAction/dodge")
				heroAction_dodge:run( cur_hero )

				local effect_dodge = require("GameLuaScript/Scene/battle/effect/dodge")
				effect_dodge:run( cur_hero )
			end

			-- 暴击
			if cur_data["actions"]["critical"] then
				local effect_critical = require("GameLuaScript/Scene/battle/effect/critical")
				effect_critical:run( cur_hero )
			end

			-- 恢复
			if cur_data["actions"]["recover"] then
				local effect_recover = require("GameLuaScript/Scene/battle/effect/recover")
		...
		if cur_data["hp"] <= 0 then
			local heroAction_die = require("GameLuaScript/Scene/battle/heroAction/die")
