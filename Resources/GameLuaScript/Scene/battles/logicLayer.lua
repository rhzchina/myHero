...
function M:create( )
	...
	-- 英雄卡牌层
	local heroLayer = require("GameLuaScript/Scene/battle/heroLayer")
	layers["hero"] = heroLayer:create()
	main_layer:addChild( layers["hero"] )

	-- 幻兽层
	local petLayer = require("GameLuaScript/Scene/battle/petLayer")
	layers["pet"] = petLayer:create( prepare_data["p1_pet"] )
	main_layer:addChild( layers["pet"] )

	-- 主人技层
	local masterSkillLayer = require("GameLuaScript/Scene/battle/masterSkillLayer")
...
function M:next()
	...
	if step_type == "skip" then
		...
		M:pause()
		local step_action = require("GameLuaScript/Scene/battle/step/" .. step_type)
