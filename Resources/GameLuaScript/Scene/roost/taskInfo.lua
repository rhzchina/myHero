...
function roostLayer:new(ksv,data,p_x,p_y)
	...
	local bg = CCSprite:create("image/scene/task/infobox.png")
	setAnchPos(bg,5,0)
	...
	local header = CCSprite:create("image/myhero/small/1001.png")
	...
	local text_name = CCLabelTTF:create(data["name"].."     执行 "..data["in_num"].."/"..data["num"], "HT.TTF" , 22)
	setAnchPos(text_name,125,135)
	this.layer:addChild(text_name)

	----金叶
	local leaf = CCSprite:create("image/scene/home/leaf.png")
	setAnchPos(leaf,140,105)
	this.layer:addChild(leaf)

	local text_leaf = CCLabelTTF:create(data["money"], "HT.TTF" , 25)
	setAnchPos(text_leaf,180,105)
	this.layer:addChild(text_leaf)

	----硬币
	local yinbi = CCSprite:create("image/scene/home/yinbi.png")
	setAnchPos(yinbi,240,105)
	this.layer:addChild(yinbi)

	local text_yinbi= CCLabelTTF:create(data["money"], "HT.TTF" , 25)
	setAnchPos(text_yinbi,280,105)
	this.layer:addChild(text_yinbi)

	----体力
	local evn = CCSprite:create("image/scene/home/env.png")
	setAnchPos(evn,140,75)
	this.layer:addChild(evn)

	local text_evn = CCLabelTTF:create(data["csenergy"], "HT.TTF" , 25)
	setAnchPos(text_evn,180,75)
	this.layer:addChild(text_evn)

	----气
	local img_qi = CCSprite:create("image/scene/home/qi.png")
	setAnchPos(img_qi,240,70)
	this.layer:addChild(img_qi)

	local text_qi = CCLabelTTF:create("100", "Arial" , 25)
	setAnchPos(text_qi,280,75)
	this.layer:addChild(text_qi)

	---描述
	local text_desc = CCLabelTTF:create(data["descs"], "HT.TTF" , 25)
