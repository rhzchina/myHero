...
--        setmetatable(this, KNBar)  --将this的元表设定为Class
        ...
        local KNBarPath = IMAGEPATH .. "/start_bar/" .. pathStr .. "/"
	    ...
	    this.KNBar:setPosition(CCPointMake(0, 0))
     	this.KNBar:setPercentage(0)	--设置默认进度值
	   	...
	   	--进度条文字
	   	this.text = CCLabelTTF:create("0/" .. this.maxValue , "Arial" , 15)
	   	...
	   	if(isset(param , "icon")) then
		   	...
		   	this.KNBar:setPosition(CCPointMake(iconSize.width/2, 0))
		   	this.bg:setPosition(ccp(iconSize.width/2 , 0))
	   		this.text:setPosition( bgSize.width / 2 + iconSize.width/2 + 5, 0 )
