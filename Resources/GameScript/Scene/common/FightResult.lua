...
function FightResult:new(type_scene,result, hero, gift)
	...
	if type_scene == "task" then
		...
		if gameover["Exp"] then
			...
			setAnchPos(silver,70,430)
			this.contentLayer:addChild(silver)
			
			local title_max = display.strokeLabel(gameover["Exp"],230,433,30,ccc3(255,255,255))
			this.contentLayer:addChild(title_max)
		end
		
		if temp["gameover"]["guanka"]["item"] then
			local scroll = ScrollView:new(40,200,480,200,10,true)
			local temp_btn
	...
	elseif type_scene == "sport" then
		local gameover = temp["gameover"]["guanka"]["tast"]
		if gameover then
			local silver = newSprite(IMG_COMMON.."shengwang.png")
			setAnchPos(silver,70,530)
			this.contentLayer:addChild(silver)
			
			local title_max = display.strokeLabel("+"..gameover["add_prestige"],130,533,30,ccc3(255,255,255))
			this.contentLayer:addChild(title_max)
			
			local title_max = display.strokeLabel("今日剩余挑战次数:"..gameover["time"]["challenge_num"],70,483,30,ccc3(255,255,255))
			this.contentLayer:addChild(title_max)
